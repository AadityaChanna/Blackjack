...
public class Picture extends SimplePicture
{
  ...
  public void encode(Picture messagePict){
    ...
    for (int row = 0; row < message.length; row ++){
      for (int col = 0; col < message[0].length; col ++){
        if (message[row][col].colorDistance(Color.BLACK) < 50){
  ...
  public Picture decode(){
    Pixel[][] codedPixel = this.getPixels2D();
    Picture decodedPict = new Picture(this);
    Pixel[][] decodedPixel = decodedPict.getPixels2D();
    for(int row = 0; row < codedPixel.length; row ++){
      for (int col = 0; col < codedPixel[0].length; col ++){
        if ((codedPixel[row][col].getRed()) % 2 != 0){
          decodedPixel[row][col].setColor(Color.BLACK);
        }
        else{
          decodedPixel[row][col].setColor(Color.WHITE);
        }
      }
    }
    return decodedPict;
